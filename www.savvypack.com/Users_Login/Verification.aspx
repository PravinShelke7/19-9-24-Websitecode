<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Verification.aspx.vb" Inherits="Users_Login_Verification" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript" language="javascript">
        function closewindow(flag, Type) {
            ...
            if (Type.toString() == "Acc") {
                if (flag == "Y") {
                    window.opener.document.getElementById('ctl00_hdnAlert').value = "1";
                }
                window.opener.document.getElementById('ctl00_ContentPlaceHolder1_btnCloseAcp').click();
            }
            else {
                window.opener.document.getElementById('btnCloseCA').click();
            }
            window.close();
            //window.open("../Index.aspx", 'NewWindow', "");           
        }

        function CloseWindowWithoutLogin(domIds, Type) {
            //window.open("../Index.aspx", 'NewWindow', "");           
            ...
            if (domIds == "2") {
                if (Type.toString() == "Acc") {
                    window.opener.document.getElementById('ctl00_ContentPlaceHolder1_btnCloseUA').click();
                }
                else {
                    window.opener.document.getElementById('btnCloseUA').click();
                }
            }
            else if (domIds == "3") {
                if (Type.toString() == "Acc") {
        ...
        function CloseWindowUType(Type) {
            window.close()
            if (Type.toString() == "Acc") {
                window.opener.document.getElementById('ctl00_ContentPlaceHolder1_btnCloseAcp').click();
            }
            else {
                window.opener.document.getElementById('btnCloseAcp').click();
            }
        }
