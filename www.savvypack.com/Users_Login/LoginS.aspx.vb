...
Partial Class Users_Login_LoginS
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            hidServ.Value = Request.QueryString("Serv").ToString()
    ...
    '                    dsVerf = objGetData.ValidateUserWithCode(txtEmail.Text.Trim(), txtPass.Text.Trim().Replace("'", "''"), txtVerfCode.Text)
    ...
    '                            Else
    '                                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "UserLogin", "closewindow('Y');", True)
    '                            End If
    '                        Else
    ...
    '                    End If
    '                Else
    '                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinRename", "alert('The verification of your email address is incomplete, and the verification must be completed to use your account.  Click on Verify Email to re-start the verification process.');", True)
    '                End If
    '            End If
    '        Else
            ...
            Else
                ...
                If dsU.Tables(0).Rows.Count > 0 Then
                    ...
                    If dsU.Tables(0).Rows(0).Item("ISVALIDEMAIL").ToString() = "Y" And dsU.Tables(0).Rows(0).Item("ISAPPROVED").ToString() = "Y" Then
                        ...
                        Session("INTUSR") = dsU.Tables(0).Rows(0).Item("ISINTERNALUSR").ToString()
                        ...
                        If dsU.Tables(0).Rows(0).Item("STATUSID").ToString() = 2 Then
                            If dsU.Tables(0).Rows(0).Item("ACCCHECKDATE").ToString() <> "" Then
                                AccCheckDays = DateDiff(DateInterval.Day, CDate(dsU.Tables(0).Rows(0).Item("ACCCHECKDATE")), Date.Now)
                                If AccCheckDays > 365 Then
                                    objUpdateData.UpdateAccCheckDate(Session("UserId"))
                                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "AccCheck", "ShowAccCheckAlert('../Users_Login/AddEditUser.aspx?Mode=" + obj.Encrypt("Edit") + "');", True)
                                Else
                                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "UserLogin", "closewindow();", True)
                                End If
                            Else
                ...
                Else
                    ...
                    If dsVer.Tables(0).Rows.Count <= 0 Then
                        ...
                        Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinRename", "alert('This username does not exist. Please create an account.');", True)
                    ElseIf dsVer.Tables(0).Rows(0).Item("USERNAME").ToString() <> "" And dsVer.Tables(0).Rows(0).Item("PASSWORD").ToString() <> "" Then
...
#Region "Send Mail"
    ...
    Protected Function GetEmailBodyData(ByVal link As String, ByVal EmailLink As String, ByVal VerfCode As String) As String
        ...
        Try
            ...
            StrSql = StrSql & "<tr> "
            StrSql = StrSql & "<td><img src='" + link + "/Images/SavvyPackLogo_EmailHead.gif' /> "
            StrSql = StrSql & "<br /> "
            StrSql = StrSql & "</td> "
            StrSql = StrSql & "</tr> "
            ...
            StrSql = StrSql & "<p>Please use this code in Verification process on SavvyPack Corporation's website.<br/>During the verification process you will be prompted to enter this code.<br/>When prompted, please enter this code, and your account will be verify.</p> "
    ...
    Protected Function GetEmailBodyData11(ByVal link As String, ByVal Name As String, ByVal USERCONTACTID As String, ByVal EMAILADDRESS As String) As String
        ...
        End Try
    End Function
    Public Sub SendEmail22(ByVal strBody As String, ByVal code As String)
        Try
            Dim objGetData As New UsersGetData.Selectdata
            Dim ds As New DataSet
                ...
                If dsVer.Tables(0).Rows(0).Item("ISVALIDEMAIL").ToString() = "Y"  And dsVer.Tables(0).Rows(0).Item("ISAPPROVED").ToString() = "Y" Then
                    ...
                    If ds.Tables(0).Rows.Count > 0 Then
						...
						'Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinB", "alert('1');", True)
                            ...
                            Session("INTUSR") = ds.Tables(0).Rows(0).Item("ISINTERNALUSR").ToString()
                            Session("Back") = "Secure"
                            Session("SBack") = "Secure"
                            hidUserID.Value = Session("UserId").ToString()
    ...
    Protected Sub lnkForgotPass_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkForgotPass.Click
        ...
        Try
            ...
            If DsUserData.Tables(0).Rows.Count > 0 Then
                ...
                'Issue Pk_2020_1 & Issue_Pk_2020_2[22May_20]
                If DsStatusIDCheck.Tables(0).Rows(0).Item("STATUSID").ToString() = 2 And DsStatusIDCheck.Tables(0).Rows(0).Item("PASSWORD").ToString() = "" Then
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinRename", "ShowAccountUpdation('../Users_Login/AddEditUser.aspx?UN=" + obj.Encrypt(txtEmail.Text) + "&Mode=" + obj.Encrypt("Comp") + "');", True)
                Else
