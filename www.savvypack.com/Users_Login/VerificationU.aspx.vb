...
Partial Class Users_Login_VerificationU
                        ...
                        If dsVerf.Tables(0).Rows.Count > 0 Then
                            ...
                            If dsEmail.Tables(0).Rows.Count > 0 Then
                                ...
                                If DsDomain.Tables(0).Rows(0).Item("ISNEW").ToString() = "Y" Or dsVerf.Tables(0).Rows(0).Item("ISNEW").ToString() = "Y" Then
                                    ...
                                    SendEmailNotify(strBody, "ACTCOMPLCD")
                                Else
                                    'Sending mail
                                    strBody = GetEmailBodyDataU(strLink, dsVerf.Tables(0).Rows(0).Item("Name").ToString(), dsVerf.Tables(0).Rows(0).Item("USERCONTACTID").ToString(), dsVerf.Tables(0).Rows(0).Item("EMAILADDRESS").ToString())
                                    SendEmail(strBody)
                                End If
                                'end
                                ...
                                Else
                                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "UserLogin", "CloseWindowWithoutLogin();", True)
                                End If
                                
                            End If
                            ''Checking for approved user
                            'If ds.Tables(0).Rows(0).Item("ISAPPROVED") = "Y" Then
                            '    Session("UserId") = ds.Tables(0).Rows(0).Item("UserID").ToString()
                            '    Session("Back") = "Secure"
                            '    objUpdateData.LoginLogOutLogInsert(dsVerf.Tables(0).Rows(0).Item("USERNAME").ToString(), Session.SessionID)
