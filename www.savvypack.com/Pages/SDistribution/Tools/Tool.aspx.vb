...
Imports ToolCCS
Partial Class Pages_SDistribution_Tools_Tool
    ...
    Protected Sub GetUserDetails()
        ...
        Dim ds As New DataSet()
        Try
            ds = objGetData.GetUserCompanyUsers(Session("SDistUserName").ToString())
            With ddlUsers
                .DataSource = ds
    ...
    Protected Sub GetTotalCaseCount()
        ...
        Dim dsSUserMaxCountEDist As New DataSet()
        ...
        Try
            ...
            dsSUserMaxCountEDist = objGetData.GetSelectedUserDetails(ddlUser2.SelectedItem.Text, "EDistribution")
            If dsSUserMaxCountSDist.Tables(0).Rows.Count > 0 Then
                hidUserMaxCaseCount.Value = CInt(dsSUserMaxCountSDist.Tables(0).Rows(0).Item("MAXCASECOUNT")).ToString()
                hidTotalSCount.Value = CInt(dsSUserCount.Tables(0).Rows(0).Item("TOTALCOUNT")).ToString()
            Else

                If dsSUserMaxCountEDist.Tables(0).Rows.Count > 0 Then
                    hidUserMaxCaseCount.Value = dsSUserMaxCountEDist.Tables(0).Rows(0).Item("MAXCASECOUNT").ToString().Trim()
            ...
            dsShareUserMaxCount = objGetData.GetSelectedUserDetails(ddlUsers.SelectedItem.Text, "SDistribution")
...
#Region "Synchronize"
    Protected Sub btnSynchronize_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSynchronize.Click
        ...
        Try
            ...
            obj.CaseSynch(SCaseId, TCaseId, "Distribution", "SDistribution", "SDistributionConnectionString", "SystemConnectionString")
            Flag = True
        Catch ex As Exception
            Flag = False
        ...
        If Flag Then
            GetPCaseDetails()
            Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinCreate", "alert('You have suceessfully synchronized Case #" + SCaseId.ToString() + " from EDist to SDist');", True)
        End If
    End Sub
#End Region
...
#Region "Rename Case"
    Protected Sub btnRename_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRename.Click
        ...
        Try
            CaseId = ddlPCases.SelectedValue.ToString()
            obj.CaseRename(ddlPCases.SelectedValue.ToString(), txtCaseDe1.Text.Trim.ToString().Replace("'", "''"), txtCaseDe2.Text.Trim.ToString().Replace("'", "''"), txtCaseDe3.Text.Trim.ToString().Replace("'", "''"), "SDistributionConnectionString")
            GetPCaseDetails()
