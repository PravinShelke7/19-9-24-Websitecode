...
Partial Class Pages_Distribution_Assumptions_PalletIn
...
#Region "MastePage Content Variables"
    ...
    Protected Sub GetMainHeadingdiv()
        ...
        MainHeading.InnerHtml = "EDistribution - Pallet Packaging Assumptions"
    End Sub
...
#Region "Browser Refresh Check"
    ...
    Protected Sub Page_PreInit(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreInit
        objRefresh = New zCon.Net.Refresh("_PAGES_Distribution_ASSUMPTIONS_PALLETIN")
    End Sub
                ...
                For j = 1 To 10
                    ...
                    Select Case j
                        ...
                        Case 2
                            ...
                            Link.Width = 150
                            Link.CssClass = "Link"
                            GetPalletDetails(Link, hid, CInt(ds.Tables(0).Rows(0).Item("M" + i.ToString() + "").ToString()))
                        ...
                        Case 8
                            InnerTdSetting(tdInner, "", "Right")
                            lbl = New Label
                            lbl.Text = FormatNumber(ds.Tables(0).Rows(0).Item("PS" + i.ToString() + "").ToString(), 2)
                            tdInner.Controls.Add(lbl)
                            trInner.Controls.Add(tdInner)
                        Case 9
                            InnerTdSetting(tdInner, "", "Center")
                            ...
                            txt.Width = 50
                            txt.ID = "P" + i.ToString()
                            txt.Text = FormatNumber(ds.Tables(0).Rows(0).Item("PP" + i.ToString() + "").ToString(), 2)
                            txt.MaxLength = 6
                            tdInner.Controls.Add(txt)
                            trInner.Controls.Add(tdInner)
                        Case 10
    ...
    Protected Sub Update_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        ...
        Try
            If Not objRefresh.IsRefresh Then
                For i = 1 To 10
                    Pallet(i - 1) = Request.Form("ctl00$DistributionContentPlaceHolder$hidPalid" + i.ToString() + "")
                    Number(i - 1) = Request.Form("ctl00$DistributionContentPlaceHolder$NUM" + i.ToString() + "").Replace(",", "")
                    NoOfUses(i - 1) = Request.Form("ctl00$DistributionContentPlaceHolder$NOFUSE" + i.ToString() + "").Replace(",", "")
                    PrefWeight(i - 1) = Request.Form("ctl00$DistributionContentPlaceHolder$W" + i.ToString() + "").Replace(",", "")
                    PrefPrice(i - 1) = Request.Form("ctl00$DistributionContentPlaceHolder$P" + i.ToString() + "").Replace(",", "")
                    Dept(i - 1) = Request.Form("ctl00$DistributionContentPlaceHolder$hidPalletDepid" + i.ToString() + "")
                    If Request.Form("ctl00$DistributionContentPlaceHolder$NEW" + i.ToString() + "") = "on" Then
                        WNew(i - 1) = "1"
                    Else
                        WNew(i - 1) = "0"
                    End If
                ...
                ObjUpIns.PalletUpdate(CaseId, Pallet, Number, NoOfUses, PrefWeight, PrefPrice, Dept, WNew)
                Calculate()
            End If
            GetPageDetails()
        Catch ex As Exception
