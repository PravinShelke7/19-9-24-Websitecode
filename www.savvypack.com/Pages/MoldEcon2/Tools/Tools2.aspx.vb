...
Imports System.Net
Partial Class Pages_MoldEcon2_Tools_Tools2
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            End If
            If Session("MoldE2UserRole") = "AADMIN" Then
    ...
    Protected Sub GetUserDetails()
        ...
        Dim ds As New DataSet()
        Try
            ds = objGetData.GetUserCompanyUsers(Session("MoldE2UserName").ToString())
            With ddlUsers
                .DataSource = ds
    ...
    Protected Sub GetTotalCaseCount()
        ...
        Try
            ...
            dsSUserMaxCountE2 = objGetData.GetSelectedUserDetails(ddlUser2.SelectedItem.Text, "MOLDE2")
            ...
            Else
                dsSUserMaxCountS2 = objGetData.GetSelectedUserDetails(ddlUser2.SelectedItem.Text, "MOLDS2")
            ...
            dsShareUserMaxCount = objGetData.GetSelectedUserDetails(ddlUsers.SelectedItem.Text, "MOLDE2")
...
#Region "Copy A Case"
    Protected Sub btnCopyBcase_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCopyBcase.Click
        ...
        Try
            ...
            Flag = CopyCase(SCaseId, TCaseId, "MoldE2ConnectionString") 'Replace EconConnectionString
            Flag = CopyCase(SCaseId, TCaseId, "MoldS2ConnectionString") 'Replace Sustain1ConnectionString
            If Flag Then
                GetPCaseDetails()
                GetTotalCaseCount()
...
#Region "Transfer A Case"
    Protected Sub btnBS2T_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBS2T.Click
        ...
        Try
            ...
            Flag = CopyCase(SCaseId, TCaseId, "MoldE2ConnectionString")
            If Flag Then
                GetPCaseDetails()
...
#Region "Share Access"
    Protected Sub btnShareAccess_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnShareAccess.Click
        ...
        Try
            'Getting details for selected user for share Access
            dsShareUserCount = objGetData.GetTotalCaseCount(hidUserSA.Value.ToUpper())
            dsShareUserMaxCount = objGetData.GetSelectedUserDetails(hidUserSA.Value, "MOLDE2")
            If dsShareUserMaxCount.Tables(0).Rows.Count > 0 Then
                hidShareUserMaxCaseCount.Value = CInt(dsShareUserMaxCount.Tables(0).Rows(0).Item("MAXCASECOUNT")).ToString()
                hidTotalShareCount.Value = CInt(dsShareUserCount.Tables(0).Rows(0).Item("TOTALCOUNT")).ToString()
            Else
            ...
            If hidTotalShareCount.Value <> "" Or hidShareUserMaxCaseCount.Value <> "" Then
                If CInt(hidTotalShareCount.Value) < CInt(hidShareUserMaxCaseCount.Value) Then
                    ...
                    obj.CaseAccess(UserName, CaseId, "MoldE2ConnectionString")
                    obj.CaseAccess(UserName, CaseId, "MoldS2ConnectionString")
...
#Region "Share A Copy"
    Protected Sub btnShareCopy_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnShareCopy.Click
        ...
        Try
            'Getting Total Case created for selected User for share copy
            dsSUserCount = objGetData.GetTotalCaseCount(hidUserSC.Value.ToUpper())
            'Getting details for selected user for share Copy
            dsSUserMaxCountE2 = objGetData.GetSelectedUserDetails(hidUserSC.Value, "MOLDE2")
            If dsSUserMaxCountE2.Tables(0).Rows.Count > 0 Then
                hidUserMaxCaseCount.Value = CInt(dsSUserMaxCountE2.Tables(0).Rows(0).Item("MAXCASECOUNT")).ToString()
                hidTotalSCount.Value = CInt(dsSUserCount.Tables(0).Rows(0).Item("TOTALCOUNT")).ToString()
            Else
                dsSUserMaxCountS2 = objGetData.GetSelectedUserDetails(hidUserSC.Value, "MOLDS2")
                If dsSUserMaxCountS2.Tables(0).Rows.Count > 0 Then
                    hidUserMaxCaseCount.Value = dsSUserMaxCountS2.Tables(0).Rows(0).Item("MAXCASECOUNT").ToString().Trim()
                    hidTotalSCount.Value = dsSUserCount.Tables(0).Rows(0).Item("TOTALCOUNT").ToString().Trim()
                Else
            ...
            If hidTotalSCount.Value <> "" Or hidUserMaxCaseCount.Value <> "" Then
                If CInt(hidTotalSCount.Value) < CInt(hidUserMaxCaseCount.Value) Then
                    ...
                    Flag = CopyCase(SCaseId, TCaseId, "MoldE2ConnectionString")
                    Flag = CopyCase(SCaseId, TCaseId, "MoldS2ConnectionString")
                    If Flag Then
                        GetPCaseDetails()
                        GetTotalCaseCount()
...
#Region "Synchronize"
    Protected Sub btnSynchronize_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSynchronize.Click
        ...
        Try
            ...
            obj.CaseSynch(SCaseId, TCaseId, "SUSTAIN2MOLD", "ECON2MOLD", "MoldE2ConnectionString", "SystemConnectionString")
            Flag = True
        Catch ex As Exception
            Flag = False
        ...
        If Flag Then
            ...
            lnkPropCase.Text = hidPropCaseD.Value
            GetPCaseDetails()
            Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinCreate", "alert('You have suceessfully synchronized Case #" + SCaseId.ToString() + " from Sustain2 Mold to Econ2 Mold');", True)
        End If
    End Sub
#End Region
...
#Region "Rename Case"
    ...
    Protected Sub btnRename_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRename.Click
        ...
        Try
            CaseId = hidPropCase.Value
            obj.CaseRename(CaseId, txtCaseDe1.Text.Trim.ToString().Replace("'", "''"), txtCaseDe2.Text.Trim.ToString().Replace("'", "''"), txtCaseDe3.Text.Trim.ToString().Replace("'", "''"), "MoldE2ConnectionString")
            GetPCaseDetails()
...
#Region "Delete Case"
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        Try
            CaseId = hidPropCase.Value
            obj.CaseDelete(Session("UserId").ToString(), CaseId.ToString(), "MoldE2ConnectionString")
            obj.CaseDelete(Session("UserId").ToString(), CaseId.ToString(), "MoldS2ConnectionString")

            'Delete Case from Group
            obj.GroupCaseDelete(Session("UserId").ToString(), CaseId.ToString(), "MoldE2ConnectionString")
...
#Region "Tools Function"
    Protected Function CreateCase(ByVal UserId As String) As Integer
        ...
        Dim obj As New ToolCCS
        Try
            CaseId = obj.CreateCase(UserId, "MoldE2ConnectionString", "MoldE2", 0)
            obj.CreateCase(UserId, "MoldS2ConnectionString", "MoldE2", CaseId)
            Return CaseId
        Catch ex As Exception
...
#End Region
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        Try
            Dim objUpdateData As New MoldE2UpInsData.UpdateInsert()
            Dim objGetData As New MoldE2GetData.Selectdata()
            Dim ds As New DataSet()
            ...
            If hidPropCase.Value = "0" Then
                ...
                lnkPropCase.Text = "Select case"
            Else
                objUpdateData.PermissionStatusUpdate(hidPropCase.Value, Session("MoldE2UserName").ToString())
    ...
    Protected Sub SendEmailStatus(ByVal CaseId As String)
        Try
            ...
            Dim objE2GetData As New MoldE2GetData.Selectdata()
            Dim ds As New DataSet()
            ...
            dsUserData = objE2GetData.GetPermissionStatus(CaseId, Session("MoldE2UserName").ToString())
            If ds.Tables(0).Rows.Count > 0 Then
    ...
    Public Function GetEmailBodyData(ByVal link As String, ByVal ds As DataSet) As String
        ...
        Try
            ...
            StrSqlBody = StrSqlBody + "<td>Econ2 Mold</b></td> "
