...
Partial Class Pages_Sustain1_Assumptions_Equipment2IN
...
#Region "MastePage Content Variables"
    ...
    Protected Sub GetUpdatebtn()
        ...
        'Updatebtn.Attributes.Add("onclick", "return CheckForEquipment2In()")
    ...
    Protected Sub GetMainHeadingdiv()
        ...
        MainHeading.InnerHtml = "Sustain1 - Support Equipment Assumptions"
    End Sub
...
#Region "Browser Refresh Check"
    ...
    Protected Sub Page_PreInit(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreInit
        objRefresh = New zCon.Net.Refresh("_PAGES_SUSTAIN1_ASSUMPTIONS_EQUIPMENT2IN")
    End Sub
                ...
                For j = 1 To 11
                    ...
                    Select Case j
                        ...
                        Case 2
                            ...
                            Link.Width = 150
                            Link.CssClass = "Link"
                            GetSupportEquipmentInDetails(Link, hid, CInt(ds.Tables(0).Rows(0).Item("M" + i.ToString() + "").ToString()), dsEquip)
                        ...
                        Case 3 'Bug# 375
                            ...
                            If ViewState("IsFirstLoad") = "N" Then
                                txt.Text = FormatNumber(Request.Form("ctl00$Sustain1ContentPlaceHolder$ASSETNUM" + i.ToString() + "").Replace(",", ""), 0)
                            Else
                        ...
                        Case 5
                            ...
                            lbl = New Label
                            lbl.Text = FormatNumber(ds.Tables(0).Rows(0).Item("ERGYS" + i.ToString() + "").ToString(), 0)
                        ...
                        Case 6
                            ...
                            txt.Text = FormatNumber(ds.Tables(0).Rows(0).Item("ERGYP" + i.ToString() + "").ToString(), 0)
                            txt.Width = 70
                            txt.MaxLength = 12
                            tdInner.Controls.Add(txt)
                            trInner.Controls.Add(tdInner)
                        Case 7
                            InnerTdSetting(tdInner, "", "Right")
                            lbl = New Label
                            lbl.Text = FormatNumber(ds.Tables(0).Rows(0).Item("NTGS" + i.ToString() + "").ToString(), 0)
                            tdInner.Controls.Add(lbl)
                            trInner.Controls.Add(tdInner)
                        Case 8
                            InnerTdSetting(tdInner, "", "Center")
                            ...
                            txt.Text = FormatNumber(ds.Tables(0).Rows(0).Item("NTGP" + i.ToString() + "").ToString(), 0)
                            txt.MaxLength = 12
                            txt.Width = 70
                            tdInner.Controls.Add(txt)
                            trInner.Controls.Add(tdInner)
                        ...
                        Case 11
                            ...
                            Link.Width = 120
                            Link.CssClass = "Link"
                            GetDeptDetails(Link, hid, CInt(ds.Tables(0).Rows(0).Item("DEPT" + i.ToString() + "").ToString()), dsDept)
                            tdInner.Controls.Add(hid)
                            tdInner.Controls.Add(Link)
                            trInner.Controls.Add(tdInner)
    ...
    Protected Sub Update_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        ...
        Try
            If Not objRefresh.IsRefresh Then
                For i = 1 To 30
                    hidAssetId(i - 1) = Request.Form("ctl00$Sustain1ContentPlaceHolder$hidAssetId" + i.ToString() + "")
                    ECP(i - 1) = Request.Form("ctl00$Sustain1ContentPlaceHolder$ECP" + i.ToString() + "").Replace(",", "")
                    NGCP(i - 1) = Request.Form("ctl00$Sustain1ContentPlaceHolder$NGCP" + i.ToString() + "").Replace(",", "")
                    WATERP(i - 1) = Request.Form("ctl00$Sustain1ContentPlaceHolder$WATERP" + i.ToString() + "").Replace(",", "")
                    HRS(i - 1) = Request.Form("ctl00$Sustain1ContentPlaceHolder$HRS" + i.ToString() + "").Replace(",", "")
                    hidAssetDep(i - 1) = Request.Form("ctl00$Sustain1ContentPlaceHolder$hidEqDepid" + i.ToString() + "")
                    ASSETNUM(i - 1) = Request.Form("ctl00$Sustain1ContentPlaceHolder$ASSETNUM" + i.ToString() + "") 'Bug #375
                ...
                Next
                ObjUpIns.SupportEquipmentUpdate(CaseId, hidAssetId, ECP, NGCP, HRS, WATERP, hidAssetDep, ASSETNUM)
                Calculate()
                'Update Server Date
