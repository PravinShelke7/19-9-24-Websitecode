...
Partial Class Pages_MoldEcon1_Assumptions_ProductFormatIn
...
#Region "MastePage Content Variables"
    ...
    Protected Sub GetMainHeadingdiv()
        ...
        MainHeading.InnerHtml = "Econ1 Mold - Product Format Assumptions"
    End Sub
...
#End Region
    ...
    Protected Sub Update()
        ...
        Try
            If Not objRefresh.IsRefresh Then
                PwtPref = Request.Form("ctl00$MoldEcon1ContentPlaceHolder$PWT").ToString()
                'Check for readonly User
                If Session("MoldE1ServiceRole") <> "ReadOnly" Then
                    UpdateFlag = 0
                    ds = ObjGetdata.GetDiscretedMaterialTotal(CaseId)
                    DiscTotal = CDbl(ds.Tables(0).Rows(0).Item("DISCRETEWT").ToString())
                    ...
                    If UpdateFlag = CDbl(0) Then
                        For i = 2 To 6
                            Input(i) = Request.Form("ctl00$MoldEcon1ContentPlaceHolder$M" + i.ToString() + "").Replace(",", "")
                            'Check For IsNumric
                            If Not IsNumeric(Input(i)) Then
                                Response.Redirect("../Errors/Error.aspx?ErrorCode=" + obj.Encrypt("ALDE111").Replace("+", "!Plus!").Replace("#", "!Hash!").Replace("&", "!And!") + "")
                            End If
