...
Imports E1GetData
Imports E1UpInsData
Partial Class Pages_Econ1_Tools_ManageApprovedGroups
    ...
    Protected Sub GetPCaseDetails()
        ...
        Try
            If Session("SavvyModType") <> "1" Then
                ds = objGetData.GetApprovedCaseGrpDetailsBem(Session("USERID").ToString(), txtKey.Text.Trim().Replace("'", "''").ToString())
            Else
                If Session("Service") = "COMP" Then
                    ds = objGetData.GetApprovedCompCaseGrpDetails(Session("USERID").ToString(), txtKey.Text.Trim().Replace("'", "''").ToString(), Session("ServiceId"))
                Else
                    ds = objGetData.GetApprovedCaseGrpDetails(Session("USERID").ToString(), txtKey.Text.Trim().Replace("'", "''").ToString())
                End If
    ...
    Protected Sub btnCreateGrp_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreateGrp.Click
        Try
            ...
            If Name.Length <> 0 Then
                ...
                dt = objGetData.ValidateApprovedGroupName(txtGroupDe1.Text.Trim().Replace("'", "''").ToString(), txtGroupDe2.Text.Trim().Replace("'", "''").ToString(), Session("UserId"), Session("Service"))
                ...
                Else
                    If Session("Service") = "COMP" Then
                        objUpdateData.AddApprovedCompGroupName(txtGroupDe1.Text.Trim().Replace("'", "''").ToString(), txtGroupDe2.Text.Trim().Replace("'", "''").ToString(), Session("UserId"), Session("ServiceId"))
                    Else
                        objUpdateData.AddApprovedGroupName(txtGroupDe1.Text.Trim().Replace("'", "''").ToString(), txtGroupDe2.Text.Trim().Replace("'", "''").ToString(), Session("UserId"))
                    End If
    ...
    Protected Sub GetGroupsForCase()
        ...
        For Each Gr As GridViewRow In grdCase.Rows
            ...
            linkBut.Attributes.Add("onclick", "return OpenGroupPopup('../Popup/ApprovedGroupDetails.aspx?groupId=" + lblGrpId.Text.Trim() + "&CaseID=" + lblCaseID.Text.Trim() + "');")
        Next
    End Sub
