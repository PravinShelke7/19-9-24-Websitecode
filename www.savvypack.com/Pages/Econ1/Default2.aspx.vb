...
Imports System.Net
Partial Class Pages_Econ1_Default2
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                ...
                hidApprovedCaseD.Value = "Select case"
                hidApprovedGroupCaseD.Value = "All Groups and All Cases"
                hidApprovedCase.Value = "0"
            End If
          
            SetToolButton()

            If Session("E1LicAdmin") = "Y" Then
                lnkApprovedGrp.Visible = True
                lnkAppCase.Visible = True
    ...
    Protected Sub SetSessions()
        ...
        Try
            ...
            Session("E1UserRole") = ds.Tables(0).Rows(0).Item("USERROLE").ToString()
            Session("E1ServiceRole") = ds.Tables(0).Rows(0).Item("SERVIECROLE").ToString()
            Session("E1UserName") = ds.Tables(0).Rows(0).Item("USERNAME").ToString()
            Session("E1ToolUserName") = ds.Tables(0).Rows(0).Item("TOOLUSERNAME").ToString()
            Session("E1MaxCaseCount") = ds.Tables(0).Rows(0).Item("MAXCASECOUNT").ToString()
            'Set for License Administrator
            Session("E1LicAdmin") = ds.Tables(0).Rows(0).Item("ISIADMINLICUSR").ToString()
            Session("E1LicenseId") = ds.Tables(0).Rows(0).Item("LICENSEID").ToString()
    ...
    Protected Sub SetToolButton()
        Try
            If Session("E1ServiceRole") <> "ReadWrite" Then
    ...
    Protected Sub btnBCase_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBCase.Click
        Try
            ...
            Else
                ...
                Session("E1CaseId") = hidApprovedCase.Value
    ...
    Protected Sub btnPCase_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPCase.Click
        ...
        Try
            ...
            Else
                Session("E1CaseId") = hidPropCase.Value ' ddlPCase.SelectedValue.ToString()
                lnkPropCase.Text = hidPropCaseD.Value
                lnkCaseGroup.Text = hidGroupCaseD.Value
                lnkApprovedCaseGroup.Text = hidApprovedGroupCaseD.Value
                Session("CaseType") = "Prop"

                CaseId = Session("E1CaseId")
                User = Session("USERID")
                dsSt = objgetdata.GetStatusForSister("E1", CaseId, User)
    ...
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            ...
            obj.CaseRename(txtCaseid.Value, txtCaseDe1.Text.Trim().ToString().Replace("'", "''"), txtCaseDe2.Text.Trim().ToString().Replace("'", "''"), txtCaseDe3.Text.Trim.ToString().Replace("'", "''"), "EconConnectionString")
            'GetPCaseDetails()
            'Group
            If hidPackSpecGrp.Value.ToString() <> "0" Then
                obj.UpdateGroup(txtCaseid.Value, hidPackSpecGrp.Value)
            End If
            If hidPackSpecCMPNY.Value.ToString() <> "0" Then
                obj.UpdateCompany(txtCaseid.Value, hidPackSpecCMPNY.Value.ToString())
    ...
    Protected Sub GetCaseDetails()
        ...
        Try
            ...
            If ds.Tables(0).Rows(0).Item("GRPDETAIL").ToString() <> "" Then
                lnkPackSpecGRP.Text = ds.Tables(0).Rows(0).Item("GRPDETAIL").ToString()
                hidPackSpecGrp.Value = ds.Tables(0).Rows(0).Item("PACKSPECGRPID").ToString()
            Else
                lnkPackSpecGRP.Text = "Select Package Specification Group"
                hidPackSpecGrp.Value = "0"
            End If
            If ds.Tables(0).Rows(0).Item("COMPANYNAME").ToString() <> "" Then
                ddlPackSpecCMPNY.Text = ds.Tables(0).Rows(0).Item("COMPANYNAME").ToString()
                hidPackSpecCMPNY.Value = ds.Tables(0).Rows(0).Item("PACKSPECCMPNYID").ToString()
            Else
                ddlPackSpecCMPNY.Text = "Select Package Specification Company"
                hidPackSpecCMPNY.Value = "0"
            End If
    ...
    Protected Sub SendEmailStatus(ByVal CaseId As String)
        Try
            ...
            dsUserData = objE1GetData.GetPermissionStatus(CaseId, Session("USERID").ToString())
            If ds.Tables(0).Rows.Count > 0 Then
