Imports System
Imports System.Data
Partial Class Pages_Econ1_PopUp_EditApprovedGroups
    ...
    Protected Sub GetReportDetails()
        ...
        Try
            If Session("Service") = "COMP" Then
                ds = objGetData.GetApprovedCompGroupDetails(Session("USERID").ToString(), "N", Session("CompServiceId").ToString())
            Else
                'ds = objGetData.GetAllAppGroupDetails(Session("USERID").ToString(), "N")
                ds = objGetData.GetApprovedGroupDetails(Session("USERID").ToString(), "N")
            End If
    ...
    Protected Sub lnkGrpId_Click(ByVal sender As Object, ByVal e As EventArgs)
        Dim grpId As String = ""
        Dim ObjUpData As New E1UpInsData.UpdateInsert()
        Try
            ...
            If Request.QueryString("groupId") <> Nothing And Request.QueryString("CaseId") <> Nothing Then
                'Code tp Update Case
                ObjUpData.UpdateAPPGroupByCaseID(Request.QueryString("groupId"), grpId, Request.QueryString("CaseId"))
            End If
    ...
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            For Each Gr As GridViewRow In grdGrpCases.Rows
                ...
                groupName(count) = txtGroupName.Text.Trim().Replace("'", "''").ToString()
                groupDes(count) = txtGroupDes.Text.Trim().Replace("'", "''").ToString()
                lblGrpId = grdGrpCases.Rows(Gr.RowIndex).FindControl("lblGroupID")
            ...
            Else
                objUpdateData.UpdateAppGroupName(groupName, groupDes, groupID, count)
            End If
        ...
        Try
            For Each Gr As GridViewRow In grdGrpCases.Rows
                ...
                If chk.Checked = True Then
                    lblGrpId = grdGrpCases.Rows(Gr.RowIndex).FindControl("lblGroupID")
                    objUpdateData.DeleteAppGroups(lblGrpId.Text)
                End If
            Next
            GetReportDetails()
        Catch ex As Exception
