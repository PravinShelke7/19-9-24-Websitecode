...
Partial Class Pages_MedSustain1_Default
...
#Region "Browser Refresh Check"
    ...
    Protected Sub Page_PreInit(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreInit
        objRefresh = New zCon.Net.Refresh("_PAGES_MEDSUSTAIN1_DEFAULT")
    End Sub
...
#End Region
        ...
        Try
            ...
            If Not IsPostBack Then
                If Session("Service") = "COMPS1" Then
                    Page.Title = "SMed1 - Competitive Module"
                    divMainHeading.InnerHtml = "SMed1 - Competitive Module - Global Manager"
    ...
    Protected Sub SetSessions()
        ...
        Try
            ...
            Session("SMed1UserRole") = ds.Tables(0).Rows(0).Item("USERROLE").ToString()
            Session("SMed1ServiceRole") = ds.Tables(0).Rows(0).Item("SERVIECROLE").ToString()
            Session("SMed1UserName") = ds.Tables(0).Rows(0).Item("USERNAME").ToString()
            Session("SMed1ToolUserName") = ds.Tables(0).Rows(0).Item("TOOLUSERNAME").ToString()
            Session("SMed1MaxCaseCount") = ds.Tables(0).Rows(0).Item("MAXCASECOUNT").ToString()
            'Set for License Administrator
            Session("SMed1LicAdmin") = ds.Tables(0).Rows(0).Item("ISIADMINLICUSR").ToString()
            Session("SMed1LicenseId") = ds.Tables(0).Rows(0).Item("LICENSEID").ToString()

            Session("SMed1ServiceId") = ds.Tables(0).Rows(0).Item("SERVICEID").ToString()
        Catch ex As Exception
                    ...
                    If dsComp.Tables(0).Rows.Count > 0 Then
                        For i = 0 To dsComp.Tables(0).Rows.Count - 1
                            ...
                            If dsComp.Tables(0).Rows(i).Item("SERVICEDE").ToString() = "CompEcon" Then
                                Session("ServiceMed1") = dsComp.Tables(0).Rows(i).Item("SERVICEID").ToString()
                            ElseIf dsComp.Tables(0).Rows(i).Item("SERVICEDE").ToString() = "CompSustain" Then
                                Session("ServiceSMed1") = dsComp.Tables(0).Rows(i).Item("SERVICEID").ToString()
                            End If
                        Next
                        Session("CompSMed1ServiceId") = serviceId
                    End If
                End If
            Else
    ...
    Protected Sub GetPCaseDetails()
        ...
        Try
            ...
            Else
                If Session("SMed1LicAdmin") = "N" Then
                    If Session("Service") = "COMPS1" Then
                ...
                Else
                    If Session("Service") = "COMPS1" Then
                        ds = objGetData.GetCompPCaseDetailsByLicense(Session("USERID").ToString(), Session("SMed1ServiceId").ToString())
                    Else
    ...
    Protected Sub GetPCaseGroupDetails()
        ...
        Try
            ...
            If Session("Service") = "COMPS1" Then
                Dts = objGetData.GetCompGroupCaseDetails(Session("USERID").ToString(), Session("CompSMed1ServiceId").ToString())
            Else
    ...
    Protected Sub SetToolButton()
        Try
            If Session("SMed1ServiceRole") <> "ReadWrite" Then
    ...
    Protected Sub btnBCase_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBCase.Click
        Try
            Session("SMed1CaseId") = ddlBaseCases.SelectedValue.ToString()
            If Session("Service") = "COMPS1" Then
    ...
    Protected Sub btnPCase_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPCase.Click
        Try
            Session("SMed1CaseId") = ddlPCase.SelectedValue.ToString()
            If Session("Service") = "COMPS1" Then
    ...
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            ...
            'GetPCaseDetails()
            obj.CaseRename(txtCaseid.Value, txtCaseDe1.Text.Trim.ToString().Replace("'", "''"), txtCaseDe2.Text.Trim.ToString().Replace("'", "''"), txtCaseDe3.Text.Trim.ToString().Replace("'", "''"), "MedSustain1ConnectionString")
