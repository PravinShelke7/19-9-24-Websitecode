...
Partial Class Pages_Schem1_Results_WaterRes2
...
#Region "MastePage Content Variables"
    ...
    Protected Sub GetComparebtn()
        ...
        Comparebtn.OnClientClick = "return Compare('Comparison.aspx?Type=" + obj.Encrypt("CWATER") + "') "
    ...
    Protected Sub GetMainHeadingdiv()
        ...
        MainHeading.Attributes.Add("onmouseover", "Tip('Customer Water Statement')")
        MainHeading.Attributes.Add("onmouseout", "UnTip()")
        MainHeading.InnerHtml = "Schem1 - Customer Water Statement "
    End Sub
...
#Region "Browser Refresh Check"
    ...
    Protected Sub Page_PreInit(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreInit
        objRefresh = New zCon.Net.Refresh("_PAGES_SCHEM1_RESULTS_WATERRES2")
    End Sub
...
#End Region
    ...
    Protected Sub GetPageDetails()
        ...
        Try
            ...
            For i = 1 To 3
                ...
                Select Case i
                    ...
                    Case 2
                        ...
                        HeaderTdSetting(tdHeader, "180px", "Total", "1")
                        Header2TdSetting(tdHeader1, "", "", "1")
                    ...
                    Case 3
                        Title = "(" + ds.Tables(0).Rows(0).Item("TITLE10").ToString() + ")"
                        HeaderTdSetting(tdHeader, "180px", "Customer Total", "1")
                        Header2TdSetting(tdHeader1, "", "", "1")
                        trHeader.Controls.Add(tdHeader)
                        Header2TdSetting(tdHeader2, "", Title, "1")
                        trHeader.Controls.Add(tdHeader)
                    ...
                    Case 4
                        Title = "(" + ds.Tables(0).Rows(0).Item("TITLE10").ToString() + ")"
                        HeaderTdSetting(tdHeader, "140px", "Water", "1")
                        Header2TdSetting(tdHeader1, "", "", "1")
                        Header2TdSetting(tdHeader2, "", Title, "1")
                        trHeader.Controls.Add(tdHeader)
                        trHeader1.Controls.Add(tdHeader1)
            ...
            'Getting Water Total
            For k = 1 To 11
                totWater = totWater + CDbl(ds.Tables(0).Rows(0).Item("T" + k.ToString() + "").ToString())
            Next
                    ...
                    Select Case j
                        ...
                        Case 3
                            ...
                            If CDbl(ds.Tables(0).Rows(0).Item("SALESVOLUMELB").ToString()) > 0.0 Then
                                If CInt(ds.Tables(0).Rows(0).Item("CUSSALESVOLUME").ToString()) <> 0 Then
                                    If i < 12 Then
                                        lbl.Text = FormatNumber(CDbl(ds.Tables(0).Rows(0).Item("T" + i.ToString() + "").ToString()) * customerSaleValue / CDbl(ds.Tables(0).Rows(0).Item("SALESVOLUMELB").ToString()), 0)
                                        arrCustomerTotal(i - 1) = lbl.Text
                                    ElseIf i = 12 Then
                                        lbl.Text = FormatNumber(totWater * customerSaleValue / CDbl(ds.Tables(0).Rows(0).Item("SALESVOLUMELB").ToString()), 0)
                                        arrCustomerTotal(i - 1) = lbl.Text
                                    ElseIf i = 13 Then
                                        lbl.Text = FormatNumber(totPurMat * customerSaleValue / CDbl(ds.Tables(0).Rows(0).Item("SALESVOLUMELB").ToString()), 0)
                                        arrCustomerTotal(i - 1) = lbl.Text
                                    ElseIf i = 14 Then
                                        lbl.Text = FormatNumber(totProcess * customerSaleValue / CDbl(ds.Tables(0).Rows(0).Item("SALESVOLUMELB").ToString()), 0)
                                        arrCustomerTotal(i - 1) = lbl.Text
                                    ElseIf i = 15 Then
                                        lbl.Text = FormatNumber(totTransp * customerSaleValue / CDbl(ds.Tables(0).Rows(0).Item("SALESVOLUMELB").ToString()), 0)
                                        arrCustomerTotal(i - 1) = lbl.Text
                                    ElseIf i = 16 Then
                                        lbl.Text = FormatNumber(totWater * customerSaleValue / CDbl(ds.Tables(0).Rows(0).Item("SALESVOLUMELB").ToString()), 0)
                                        arrCustomerTotal(i - 1) = lbl.Text
                        ...
                        Case 4
                            ...
                            If arrCustomerTotal(i - 1) <> "na" Then
                                If IsNumeric(arrCustomerTotal(i - 1)) Then
                                    lbl.Text = FormatNumber(CDbl(arrCustomerTotal(i - 1)) / CDbl(22.5), 1)
                                End If
                            Else
                                lbl.Text = "na"
                            End If
