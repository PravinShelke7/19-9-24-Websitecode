...
Partial Class Pages_Sustain4_Assumptions_ExtraProcess
...
#End Region
    ...
    Protected Sub GetPageDetails()
        ...
        Try
            ...
            'ds = objGetData.ExtraProcess(CaseIds, UserName)
            ...
            For i = 0 To DataCnt
                ds = objGetData.GetExtraProcessDetails(arrCaseID(i))
                ...
                For j = 1 To 6
                    ...
                    Select Case j
                        Case 1
                            ...
                            LeftTdSetting(tdInner, "<b>" + Name + "</b>", trInner, "AlterNateColor4")
                            For k = 0 To DataCnt
                                'Break
                                tdInner = New TableCell
                        ...
                        Case 2
                            tdInner = New TableCell
                            Title = "(" + dstbl.Tables(0).Rows(0).Item("Title8") + " or " + dstbl.Tables(0).Rows(0).Item("Title10") + ")"
                            LeftTdSetting(tdInner, "Extra-process Waste Value " + Title + "", trInner, "AlterNateColor1")
                            trInner.ID = "VA_" + i.ToString()
                            For k = 0 To DataCnt
                                tdInner = New TableCell
                                tdInner.Text = FormatNumber(dstbl.Tables(k).Rows(0).Item("EP" + i.ToString() + "").ToString(), 0)
                                InnerTdSetting(tdInner, DWidth, "Right")
                                trInner.Controls.Add(tdInner)
                            Next
                        Case 3
                            tdInner = New TableCell
                            LeftTdSetting(tdInner, "Extra-process Waste Rule", trInner, "AlterNateColor2")
                            trInner.ID = "RUL_" + i.ToString()
                            For k = 0 To DataCnt
                                tdInner = New TableCell
                                tdInner.Text = "weight per employee"
                                If (i = 3) Then tdInner.Text = "volume per employee"
                                InnerTdSetting(tdInner, DWidth, "Right")
                                trInner.Controls.Add(tdInner)
                            Next
                        Case 4
                            ...
                            trInner.ID = "SUG_" + i.ToString()
                            For k = 0 To DataCnt
                                tdInner = New TableCell
                                tdInner.Text = FormatNumber(dstbl.Tables(k).Rows(0).Item("SUG" + i.ToString() + "").ToString(), 0)
                                InnerTdSetting(tdInner, DWidth, "Right")
                                trInner.Controls.Add(tdInner)
                            Next
                        Case 5
                            ...
                            trInner.ID = "PREF_" + i.ToString()
                            For k = 0 To DataCnt
                                tdInner = New TableCell
                                tdInner.Text = FormatNumber(dstbl.Tables(k).Rows(0).Item("PREF" + i.ToString() + "").ToString(), 0)
                                InnerTdSetting(tdInner, DWidth, "Right")
                                trInner.Controls.Add(tdInner)
                            Next
                        Case 6
