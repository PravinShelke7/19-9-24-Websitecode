...
Partial Class Pages_Econ4_Results_ResultsCostD2
...
#Region "MastePage Content Variables"
    ...
    Protected Sub GetComparebtn()
        ...
        Comparebtn.OnClientClick = "return Compare('../../../Charts/E4Charts/CPrftCost.aspx?CaseId=" + obj.Encrypt(AssumptionId) + "&PrftCost=" + objCryptoHelper.Encrypt("COST") + "&ChartType=" + objCryptoHelper.Encrypt("RBC") + "&IsDep=" + objCryptoHelper.Encrypt("Y") + "&CType=" + objCryptoHelper.Encrypt("PWeight") + "') "
...
#End Region
    ...
    Protected Sub GetPageDetails()
        ...
        Try
            ...
            For k = 0 To DataCnt
                tdInner = New TableCell
                Title = "(" + dstbl.Tables(k).Rows(0).Item("TITLE2").ToString() + "/" + dstbl.Tables(k).Rows(0).Item("TITLE8").ToString() + ")"
                tdInner.Text = Title
                    ...
                    Select Case j
                        Case 1
                            ...
                            For k = 0 To DataCnt
                                ...
                                salesVolume = CDbl(dstbl.Tables(k).Rows(0).Item("SALESVOLUMELB").ToString()) / CDbl(dstbl.Tables(k).Rows(0).Item("CONVWT").ToString())

                                If salesVolume > 1 Then
                                    perwt = CDbl(dstbl.Tables(k).Rows(0).Item("PL" + i.ToString() + "").ToString()) / salesVolume * CDbl(dstbl.Tables(k).Rows(0).Item("CURR").ToString()) / CDbl(dstbl.Tables(k).Rows(0).Item("CONVWT").ToString())
                                    tdInner.Text = FormatNumber(perwt, 3)
                                End If
                                InnerTdSetting(tdInner, "", "Right")
                                trInner.Controls.Add(tdInner)
                            Next
