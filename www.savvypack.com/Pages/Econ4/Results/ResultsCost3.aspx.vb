...
Partial Class Pages_Econ4_Results_ResultsCost3
...
#Region "MastePage Content Variables"
    ...
    Protected Sub GetComparebtn()
        ...
        Comparebtn.OnClientClick = "return Compare('../../../Charts/E4Charts/CPrftCost.aspx?CaseId=" + obj.Encrypt(AssumptionId) + "&PrftCost=" + objCryptoHelper.Encrypt("COST") + "&ChartType=" + objCryptoHelper.Encrypt("RBC") + "&IsDep=" + objCryptoHelper.Encrypt("N") + "&CType=" + objCryptoHelper.Encrypt("PUnit") + "') "
    ...
    Protected Sub GetPageDetails()
        ...
        Try
            ...
            For i = 0 To DataCnt
                ...
                ds = objGetData.GetCostDetails(arrCaseID(i), False)
            ...
            For i = 0 To 1
                ...
                For k = 0 To DataCnt
                    ...
                    'End If
                    If i = 0 Then
                        If dstbl.Tables(k).Rows(0).Item("SALESVOLUMELB").ToString() <> Nothing Then
                            str = FormatNumber(dstbl.Tables(k).Rows(0).Item("SALESVOLUMELB").ToString(), 0).ToString()
                        Else
                            str = 0
                        End If
                    Else
                        If dstbl.Tables(k).Rows(0).Item("SALESVOLUMEUNIT").ToString() <> Nothing Then
                            str = FormatNumber(dstbl.Tables(k).Rows(0).Item("SALESVOLUMEUNIT").ToString(), 0).ToString()
                        Else
                            str = 0
                        End If
            ...
            For k = 0 To DataCnt
                ...
                If CInt(dstbl.Tables(k).Rows(0).Item("finvolmsi").ToString()) > 0 Then
                    Title = "(" + dstbl.Tables(k).Rows(0).Item("title6").ToString() + " per " + dstbl.Tables(k).Rows(0).Item("title3").ToString() + ")"
                ElseIf CInt(dstbl.Tables(k).Rows(0).Item("finvolmunits").ToString()) > 0 Then
                    Title = "(" + dstbl.Tables(k).Rows(0).Item("title6").ToString() + " per unit)"
                End If
                tdInner.Text = Title
                    ...
                    Select Case j
                        Case 1
                            ...
                            For k = 0 To DataCnt
                                ...
                                '    tdInner.Text = FormatNumber(perunit, 2)
                                'End If
                                If CInt(dstbl.Tables(k).Rows(0).Item("finvolmsi").ToString()) > 0 Then
                                    perunit = CDbl(dstbl.Tables(k).Rows(0).Item("PL" + i.ToString() + "").ToString()) / CDbl(dstbl.Tables(k).Rows(0).Item("finvolmsi").ToString()) * 100 * CDbl(dstbl.Tables(k).Rows(0).Item("CURR").ToString()) / CDbl(dstbl.Tables(k).Rows(0).Item("convarea").ToString())
                                    tdInner.Text = FormatNumber(perunit, 4)
                                ElseIf CInt(dstbl.Tables(k).Rows(0).Item("finvolmunits").ToString()) > 0 Then
                                    perunit = CDbl(dstbl.Tables(k).Rows(0).Item("PL" + i.ToString() + "").ToString()) / CDbl(dstbl.Tables(k).Rows(0).Item("finvolmunits").ToString()) * 100 * CDbl(dstbl.Tables(k).Rows(0).Item("CURR").ToString())
                                    tdInner.Text = FormatNumber(perunit, 4)
                                End If
