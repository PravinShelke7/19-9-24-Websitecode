...
Partial Class Pages_Econ4_Results_ResultsCost1
...
#Region "MastePage Content Variables"
    ...
    Protected Sub GetComparebtn()
        ...
        Comparebtn.OnClientClick = "return Compare('../../../Charts/E4Charts/CPrftCost.aspx?CaseId=" + obj.Encrypt(AssumptionId) + "&PrftCost=" + objCryptoHelper.Encrypt("COST") + "&ChartType=" + objCryptoHelper.Encrypt("RBC") + "&IsDep=" + objCryptoHelper.Encrypt("N") + "&CType=" + objCryptoHelper.Encrypt("Total") + "') "
                        ...
                        Case 1
                            ...
                            For k = 0 To DataCnt
                                ...
                                For cnt = 0 To 1
                                    ...
                                    'str = FormatNumber(CDbl(dstbl.Tables(0).Rows(0).Item("PL" + i.ToString() + "").ToString()) * CDbl(dstbl.Tables(0).Rows(0).Item("CURR").ToString()), 0)
                                    'td.Text = str

                                    If cnt = 0 Then
                                        td.Text = FormatNumber(CDbl(dstbl.Tables(k).Rows(0).Item("PL" + i.ToString() + "").ToString()) * CDbl(dstbl.Tables(k).Rows(0).Item("CURR").ToString()), 0)
                                    Else
                                        Dim percentage As New Decimal
                                        If CDbl(dstbl.Tables(k).Rows(0).Item("PL24").ToString()) > 0 Then
                                            percentage = CDbl(dstbl.Tables(k).Rows(0).Item("PL" + i.ToString() + "").ToString()) / CDbl(dstbl.Tables(k).Rows(0).Item("PL24").ToString()) * 100
                                            td.Text = FormatNumber(percentage, 2)
                                        End If
                                    End If
