<%@ Page Language="VB" MasterPageFile="~/Masters/Econ3.master" AutoEventWireup="false"
    CodeFile="Equipment2IN.aspx.vb" Inherits="Pages_Econ3_Assumptions_Equipment2IN"
    Title="E3-Support Equipment Assumptions" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Econ3ContentPlaceHolder" runat="Server">
    ...
    <script>
        function removeSession() {
            ...
            document.cookie = "U8=;domain=.savvypack.com;path=/;expires=Fri, 19 Jun 2000 20:47:11 UTC;";
        }

        window.setInterval(function () {

//            if (localStorage.getItem("A8") != null) {
//                localStorage.removeItem("A8");
//                location.reload();
    ...
    <script type="text/JavaScript">
     ...
     <script type="text/javascript">
         function UpdateAll(caseId1, caseId2, caseId3, caseId4, caseId5, caseId6, caseId7, caseId8, caseId9, caseId10, countVal) {
             if (CheckForSEquipmentPageE3All('ctl00_Econ3ContentPlaceHolder', 'hidAssetId', 'hidCTdesid', 'hypCTdes', 'NOASSET', countVal)) {
                 ...
                 for (m = 0; m < countVal; m++) {
                     ...
                     for (i = 0; i < 30; i++) {
                         ...
                         arrHours[i] = document.getElementById("ctl00_Econ3ContentPlaceHolder_NOHR" + (i + 1) + "_" + eval(index)).value;
                         arrMDep[i] = document.getElementById("ctl00_Econ3ContentPlaceHolder_hidHRdesid" + (i + 1) + "_" + eval(index)).value;
                     ...
                     if (m == 0) {
                         ...
                         if (caseId1 > 1000) {
                             PageMethods.UpdateAllCases(caseId1, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
                     else if (m == 1) {
                         if (caseId2 > 1000) {
                             PageMethods.UpdateAllCases(caseId2, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
                     else if (m == 2) {
                         if (caseId3 > 1000) {
                             PageMethods.UpdateAllCases(caseId3, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
                     else if (m == 3) {
                         if (caseId4 > 1000) {
                             PageMethods.UpdateAllCases(caseId4, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }

                     else if (m == 4) {
                         if (caseId5 > 1000) {
                             PageMethods.UpdateAllCases(caseId5, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
                     else if (m == 5) {
                         if (caseId6 > 1000) {
                             PageMethods.UpdateAllCases(caseId6, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
                     else if (m == 6) {
                         if (caseId7 > 1000) {
                             PageMethods.UpdateAllCases(caseId7, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
                     else if (m == 7) {
                         if (caseId8 > 1000) {
                             PageMethods.UpdateAllCases(caseId8, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
                     else if (m == 8) {
                         if (caseId9 > 1000) {
                             PageMethods.UpdateAllCases(caseId9, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
                     else if (m == 9) {
                         if (caseId10 > 1000) {
                             PageMethods.UpdateAllCases(caseId10, index, flag, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceedAll, onErrorAll);
                         }
                     }
		...
		nfObject = new Intl.NumberFormat('en-US'); 
             for (i = 0; i < 30; i++) {
                 ...
                 var NUMN  = parseFloat(NUM.replace(/,/g, ''));
                 document.getElementById("ctl00_Econ3ContentPlaceHolder_NOASSET" + (i + 1) + "_" + eval(index)).value = nfObject.format(NUMN); //addCommas(parseFloat(NUM.replace(',', '')).toFixed(0));

                 var COST = document.getElementById("ctl00_Econ3ContentPlaceHolder_ASCOSTP" + (i + 1) + "_" + eval(index)).value;
                 var COSTN  = parseFloat(COST.replace(/,/g, ''));
                 document.getElementById("ctl00_Econ3ContentPlaceHolder_ASCOSTP" + (i + 1) + "_" + eval(index)).value = nfObject.format(COSTN); //addCommas(parseFloat(COST.replace(',', '')).toFixed(0));

                 var COST1 = document.getElementById("ctl00_Econ3ContentPlaceHolder_NOHR" + (i + 1) + "_" + eval(index)).value;
                 var COST1N  = parseFloat(COST1.replace(/,/g, ''));
                 document.getElementById("ctl00_Econ3ContentPlaceHolder_NOHR" + (i + 1) + "_" + eval(index)).value = nfObject.format(COST1N); //addCommas(parseFloat(COST1.replace(',', '')).toFixed(0));

                 document.getElementById("ctl00_Econ3ContentPlaceHolder_ASCOSTS" + (i + 1) + "_" + index + "").innerHTML = valArray[0];
                 document.getElementById("ctl00_Econ3ContentPlaceHolder_ECS" + (i + 1) + "_" + index + "").innerHTML = valArray[1];
                 document.getElementById("ctl00_Econ3ContentPlaceHolder_NGCS" + (i + 1) + "_" + index + "").innerHTML = valArray[2];
    ...
    <script type="text/javascript">
        ...
        function Update(caseId, index) {
            // alert(index);
            if (CheckForSEquipmentPageE3('ctl00_Econ3ContentPlaceHolder', 'hidAssetId', 'hidCTdesid', 'hypCTdes', 'NOASSET', index)) {
                ...
                PageMethods.UpdateCase(caseId, index, arrEQ, arrNUM, arrPCOST, arrCostType, arrHours, arrMDep, onSucceed, onError);
                return false;
            }
            else {
                return false;
    ...
    <table width="840px">
        <tr align="left">
            <td style="width: 43%" class="PageHeading" onmouseover="Tip('Support Equipment Assumptions')"
                onmouseout="UnTip()">
                Econ3 - Support Equipment Assumptions
            </td>
            ...
            <td>
                ...
                <div id="divContent2" class="divContent2">
                    <div style="margin-left: 7px; margin-top: 10px; margin-bottom: 5px">
                        ...
                        Description
                        <br />
                        <input type="checkbox" id="ctl00_Econ3ContentPlaceHolder_MAXHR" checked="checked"
                            onclick="showhideALL2('ctl00_Econ3ContentPlaceHolder_MAXHR'); " />Maximum Annual
                        Run Hours.
                        <br />
