<%@ Page Language="VB" MasterPageFile="~/Masters/Econ3.master" AutoEventWireup="false"
    CodeFile="EquipmentIN.aspx.vb" Inherits="Pages_Econ3_Assumptions_EquipmentIN"
    Title="E3-Equipment Assumptions" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Econ3ContentPlaceHolder" runat="Server">
    ...
    <script>
        function removeSession() {
            ...
            document.cookie = "U7=;domain=.savvypack.com;path=/;expires=Fri, 19 Jun 2000 20:47:11 UTC;";
        }
...
//                location.reload();
            ...
            if (CheckForPEquipmentPageE3All('ctl00_Econ3ContentPlaceHolder', 'hidAssetId', 'hidEqDepid', 'hypEqDep', 'NOASSET', countVal)) {
                ...
                for (m = 0; m < countVal; m++) {
                    ...
                    for (i = 0; i < 30; i++) {
                        ...
                        arrPAP[i] = document.getElementById("ctl00_Econ3ContentPlaceHolder_txtPARP" + (i + 1) + "_" + eval(index)).value;
                        arrECP[i] = document.getElementById("ctl00_Econ3ContentPlaceHolder_txtECP" + (i + 1) + "_" + eval(index)).value;
                        arrNGCP[i] = document.getElementById("ctl00_Econ3ContentPlaceHolder_txtNGCP" + (i + 1) + "_" + eval(index)).value;
                    ...
                    if (m == 0) {
                        ...
                        if (caseId1 > 1000) {
                            PageMethods.UpdateAllCases(caseId1, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                    else if (m == 1) {
                        if (caseId2 > 1000) {
                            PageMethods.UpdateAllCases(caseId2, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                    else if (m == 2) {
                        if (caseId3 > 1000) {
                            PageMethods.UpdateAllCases(caseId3, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                    else if (m == 3) {
                        if (caseId4 > 1000) {
                            PageMethods.UpdateAllCases(caseId4, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                        ...
                        if (caseId5 > 1000) {
                            PageMethods.UpdateAllCases(caseId5, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                    else if (m == 5) {
                        if (caseId6 > 1000) {
                            PageMethods.UpdateAllCases(caseId6, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                    else if (m == 6) {
                        if (caseId7 > 1000) {
                            PageMethods.UpdateAllCases(caseId7, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                    else if (m == 7) {
                        if (caseId8 > 1000) {
                            PageMethods.UpdateAllCases(caseId8, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                    else if (m == 8) {
                        if (caseId9 > 1000) {
                            PageMethods.UpdateAllCases(caseId9, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
                    else if (m == 9) {
                        if (caseId10 > 1000) {
                            PageMethods.UpdateAllCases(caseId10, index, flag, arrEQ, arrNUM, arrPCOST, arrDeptT, arrPAP, arrECP, arrNGCP, onSucceedAll, onErrorAll);
                        }
                    }
	 ...
	 nfObject = new Intl.NumberFormat('en-US'); 
            for (i = 0; i < 30; i++) {
               ...
               var COST = document.getElementById("ctl00_Econ3ContentPlaceHolder_ASCOSTP" + (i + 1) + "_" + eval(index)).value;
                ...
                var COST1 = document.getElementById("ctl00_Econ3ContentPlaceHolder_txtPARP" + (i + 1) + "_" + eval(index)).value;
                var COST1N  = parseFloat(COST1.replace(/,/g, ''));
                document.getElementById("ctl00_Econ3ContentPlaceHolder_txtPARP" + (i + 1) + "_" + eval(index)).value = nfObject.format(COST1N); //addCommas(parseFloat(COST1.replace(',', '')).toFixed(0));

                var COST2 = document.getElementById("ctl00_Econ3ContentPlaceHolder_txtECP" + (i + 1) + "_" + eval(index)).value;
                var COST2N  = parseFloat(COST2.replace(/,/g, ''));
                document.getElementById("ctl00_Econ3ContentPlaceHolder_txtECP" + (i + 1) + "_" + eval(index)).value = nfObject.format(COST2N); //addCommas(parseFloat(COST2.replace(',', '')).toFixed(0));

                var COST3 = document.getElementById("ctl00_Econ3ContentPlaceHolder_txtNGCP" + (i + 1) + "_" + eval(index)).value;
                var COST3N  = parseFloat(COST3.replace(/,/g, ''));
                document.getElementById("ctl00_Econ3ContentPlaceHolder_txtNGCP" + (i + 1) + "_" + eval(index)).value = nfObject.format(COST3N); //addCommas(parseFloat(COST3.replace(',', '')).toFixed(0));
    ...
    <script type="text/javascript">
        ...
        function Update(caseId, index) {
            ...
            PageMethods.UpdateCase(caseId, index,arrEQ,arrNUM,arrPCOST,arrDeptT,arrPAP,arrECP,arrNGCP, onSucceed, onError);
            return false;
            }
            else
            {
