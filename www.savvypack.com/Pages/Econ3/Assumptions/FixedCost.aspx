<%@ Page Language="VB" MasterPageFile="~/Masters/Econ3.master" AutoEventWireup="false" CodeFile="FixedCost.aspx.vb" Inherits="Pages_Econ3_Assumptions_FixedCost" title="E3-Fixed Cost Guidelines" %>
    ...
    <style type="text/css">
       ...
       <script>
           function removeSession() {
               ...
               document.cookie = "U_14=;domain=.savvypack.com;path=/;expires=Fri, 19 Jun 2000 20:47:11 UTC;";
           }

           window.setInterval(function () {

//               if (localStorage.getItem("A14") != null) {
//                   localStorage.removeItem("A14");
//                   location.reload();
...
<script type="text/javascript">
      ...
      if(CheckForFixedCostE3All('ctl00_Econ3ContentPlaceHolder','FIXCOST','FCPREF',countVal))
           {
           ...
           //alert(SessionID+'-----'+CompID);   
              ...
              for (m = 0; m < countVal; m++) {
                  var flag;
                   ...
                   arrFIXCOST[i] = document.getElementById("ctl00_Econ3ContentPlaceHolder_FIXCOST" + (i + 1) + "_" + eval(index)).value;
                   arrFCPREF[i] = document.getElementById("ctl00_Econ3ContentPlaceHolder_FCPREF" + (i + 1) + "_" + eval(index)).value;
                  ...
                  if (m == 0) {
                   if(caseId1>1000)
                      {
                      PageMethods.UpdateAllCases(caseId1, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                      }
                  }
                  else if (m == 1) {
                  if(caseId2>1000)
                      {
                      PageMethods.UpdateAllCases(caseId2, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                      }
                  }
                  else if (m == 2) {
                  if(caseId3>1000)
                      {
                       PageMethods.UpdateAllCases(caseId3, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                       }
                  }
                  else if (m == 3) {
                    if(caseId4>1000)
                      {
                       PageMethods.UpdateAllCases(caseId4, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                       }
                  }
                   ...
                   if(caseId5>1000)
                      {
                       PageMethods.UpdateAllCases(caseId5, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                       }
                  }
                  else if (m == 5) {
                   if(caseId6>1000)
                      {
                       PageMethods.UpdateAllCases(caseId6, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                       }
                  }
                  else if (m == 6) {
                    if(caseId7>1000)
                      {
                       PageMethods.UpdateAllCases(caseId7, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                       }
                  }
                  else if (m == 7) {
                   if(caseId8>1000)
                      {
                       PageMethods.UpdateAllCases(caseId8, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                       }
                  }
                  else if (m == 8) {
                   if(caseId9>1000)
                      {
                       PageMethods.UpdateAllCases(caseId9, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                       }
                  }
                  else if (m == 9) {
                   if(caseId10>1000)
                      {
                       PageMethods.UpdateAllCases(caseId10, index, flag,arrFIXCOST, arrFCPREF,arrMDep, onSucceedAll, onErrorAll);
                       }
                  }
       ...
       function onSucceedAll(result) {
            ...
            var i;
             ...
             var cpArea = [];
                      ...
                      var FIXCOSTN=document.getElementById("ctl00_Econ3ContentPlaceHolder_FIXCOST" + (i + 1) + "_" + eval(index)).value;
                        if (FIXCOSTN!=""){
                        var FIXCOSTN1  = parseFloat(FIXCOSTN.replace(/,/g, ''));
               var FIXCOSTN2 = Number(parseFloat(FIXCOSTN1).toFixed(2)).toLocaleString('en', {
                        minimumFractionDigits: 2
                    })
                      document.getElementById("ctl00_Econ3ContentPlaceHolder_FIXCOST" + (i + 1) + "_" + eval(index)).value=Number(parseFloat(FIXCOSTN1).toFixed(2)).toLocaleString('en', {
                        minimumFractionDigits: 2
                    })//addCommas(parseFloat(FIXCOSTN.replace(',','')).toFixed(2));
                      }
                      var FCPREFN=document.getElementById("ctl00_Econ3ContentPlaceHolder_FCPREF" + (i + 1) + "_" + eval(index)).value;
                       var FCPREFN1  = parseFloat(FCPREFN.replace(/,/g, ''));
                      document.getElementById("ctl00_Econ3ContentPlaceHolder_FCPREF" + (i + 1) + "_" + eval(index)).value=nfObject.format(FCPREFN1);//addCommas(parseFloat(FCPREFN.replace(',','')).toFixed(0));

                        document.getElementById("ctl00_Econ3ContentPlaceHolder_FCSUG" + (i + 1) + "_" + index + "").innerHTML = cpArea[0]; 
               }
 ...
 <script type="text/javascript">
        function clickButton(e) {
            ...
            function Update(caseId, index) {
                 if(CheckForFixedCostE3('ctl00_Econ3ContentPlaceHolder','FIXCOST','FCPREF',index))
           {
                var SessionID="<%=Session("E3SessionID")%>";   
             var CompID="<%=Session("AssumptionID")%>"; 
...
//             alert('PageMethods.UpdateCase(arr, caseId, index,SessionID,CompID, arrPosDes, arrNoWorker, arrPrefSal, onSucceed, onError);');
             ...
             PageMethods.UpdateCase(caseId, index, arrFIXCOST, arrFCPREF,arrMDep, onSucceed, onError);
...
//                      var FIXCOSTN=document.getElementById("ctl00_Econ3ContentPlaceHolder_FIXCOST" + (i + 1) + "_" + eval(index)).value;
//                     if (FIXCOSTN!=""){
//                      document.getElementById("ctl00_Econ3ContentPlaceHolder_FIXCOST" + (i + 1) + "_" + eval(index)).value=addCommas(parseFloat(FIXCOSTN.replace(',','')).toFixed(2));
//                      }
//                      var FCPREFN=document.getElementById("ctl00_Econ3ContentPlaceHolder_FCPREF" + (i + 1) + "_" + eval(index)).value;
//                      document.getElementById("ctl00_Econ3ContentPlaceHolder_FCPREF" + (i + 1) + "_" + eval(index)).value=addCommas(parseFloat(FCPREFN.replace(',','')).toFixed(0));
 ...
 var FIXCOSTN=document.getElementById("ctl00_Econ3ContentPlaceHolder_FIXCOST" + (i + 1) + "_" + eval(index)).value;
               ...
               var FIXCOSTN2 = Number(parseFloat(FIXCOSTN1).toFixed(2)).toLocaleString('en', {
                      ...
                      var FCPREFN=document.getElementById("ctl00_Econ3ContentPlaceHolder_FCPREF" + (i + 1) + "_" + eval(index)).value;
                       var FCPREFN1  = parseFloat(FCPREFN.replace(/,/g, ''));
                      document.getElementById("ctl00_Econ3ContentPlaceHolder_FCPREF" + (i + 1) + "_" + eval(index)).value=nfObject.format(FCPREFN1);
...
<asp:ScriptManager ID="ScriptManager1" runat="server" EnablePageMethods="true">
    ...
    <table width="840px">
            <tr align="left">
                <td style="width:43%" class="PageHeading" onmouseover="Tip('Fixed Cost Assumptions')" onmouseout="UnTip()" >
                  Econ3 - Fixed Cost Assumptions
                </td>
            ...
            <td>
                 ...
                 <div id="divContent2" class="divContent2"> 
                        <div style="margin-left:7px;margin-top:10px;margin-bottom:5px">
                            ...
                            <b>Categories</b>
                             <br />
                          <input type="checkbox" id="ctl00_Econ3ContentPlaceHolder_CT" checked="checked"   onclick="showhideALL2('ctl00_Econ3ContentPlaceHolder_CT'); " />Category
                             <br />
                          <input type="checkbox" id="ctl00_Econ3ContentPlaceHolder_FGVS" checked="checked"  onclick="showhideALL2('ctl00_Econ3ContentPlaceHolder_FGVS'); " />Fixed Cost Guidelines Value Sugg.
                             <br />      
                          <input type="checkbox" id="ctl00_Econ3ContentPlaceHolder_FGV" checked="checked"  onclick="showhideALL2('ctl00_Econ3ContentPlaceHolder_FGV'); " />Fixed Cost Guidelines Value Pref.
                             <br />                            
                          <input type="checkbox" id="ctl00_Econ3ContentPlaceHolder_RUL" checked="checked"  onclick="showhideALL2('ctl00_Econ3ContentPlaceHolder_RUL'); " />Rule
                             <br />
                          <input type="checkbox" id="ctl00_Econ3ContentPlaceHolder_FCS" checked="checked"  onclick="showhideALL2('ctl00_Econ3ContentPlaceHolder_FCS'); " />Fixed Cost Sugg. 
                             <br />
                          <input type="checkbox" id="ctl00_Econ3ContentPlaceHolder_FCP" checked="checked"  onclick="showhideALL2('ctl00_Econ3ContentPlaceHolder_FCP'); " />Fixed Cost Pref. 
                             <br />
                          <input type="checkbox" id="ctl00_Econ3ContentPlaceHolder_MD" checked="checked"  onclick="showhideALL2('ctl00_Econ3ContentPlaceHolder_MD'); " />Mfg. Department 
                        </div>   
