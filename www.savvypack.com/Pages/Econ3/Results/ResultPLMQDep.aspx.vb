...
Partial Class Pages_Econ3_Results_ResultPLMQDep
...
#Region "Grid"
    Public Sub GridDet()
        ...
        Try
            ...
            dsEqid = objGetData.GetPackSpecGrpDataWeightQuarterlyDep(grpComma, CountArrB, DateCount, Session("USerId"))
                ...
                For i = 0 To DateCount - 1
                    If (i Mod 3 = 0) Then
                        If dsEqid.Tables(0).Rows(Gr.RowIndex).Item("D" + QCol.ToString()).ToString() <> "" Then
                            Gr.Cells(QCol - 1).Text = FormatNumber(dsEqid.Tables(0).Rows(Gr.RowIndex).Item("D" + QCol.ToString()).ToString(), 3)
                        End If
                        QCol = QCol + 1
                    End If
                                ...
                                If (i Mod 3 = 0) Then
                                    If i <= count Then
                                        If dstbl.Tables(arrCaseID(i)).Rows(0).Item("PL" + (l + 1).ToString()).ToString() <> "" Then
                                            cost1 = FormatNumber((CDbl(dstbl.Tables(arrCaseID(i)).Rows(0).Item("PL" + (l + 1).ToString()).ToString()) * CDbl(dstbl.Tables(arrCaseID(i)).Rows(0).Item("CURR").ToString())) / (CDbl(dstbl.Tables(arrCaseID(i)).Rows(0).Item("SVOLUME").ToString())), 3)
                                        End If
                                    End If
                                    If (i + 1) <= count Then
                                        If dstbl.Tables(arrCaseID(i + 1)).Rows(0).Item("PL" + (l + 1).ToString()).ToString() <> "" Then
                                            cost2 = FormatNumber((CDbl(dstbl.Tables(arrCaseID(i + 1)).Rows(0).Item("PL" + (l + 1).ToString()).ToString()) * CDbl(dstbl.Tables(arrCaseID(i + 1)).Rows(0).Item("CURR").ToString())) / (CDbl(dstbl.Tables(arrCaseID(i + 1)).Rows(0).Item("SVOLUME").ToString())), 3)
                                        End If
                                    End If
                                    If (i + 2) <= count Then
                                        If dstbl.Tables(arrCaseID(i + 2)).Rows(0).Item("PL" + (l + 1).ToString()).ToString() <> "" Then
                                            cost3 = FormatNumber((CDbl(dstbl.Tables(arrCaseID(i + 2)).Rows(0).Item("PL" + (l + 1).ToString()).ToString()) * CDbl(dstbl.Tables(arrCaseID(i + 2)).Rows(0).Item("CURR").ToString())) / (CDbl(dstbl.Tables(arrCaseID(i + 2)).Rows(0).Item("SVOLUME").ToString())), 3)
                                        End If
                                    End If
