...
Partial Class Pages_Contract_LogicSearch
...
#Region "MastePage Content Variables"
    ...
    Protected Sub GetMainHeadingdiv()
        ...
        MainHeading.InnerHtml = "Contract - Logic Search Results"
    End Sub
...
#End Region
    ...
    Protected Sub GetPageDetails()
        ...
        Try
            dsLogicOR = objGetdata.GetLogicORSearchResults(Session("USERID").ToString())
                    ...
                    Select Case j
                        Case 1
                            ...
                            If dsLogicOR.Tables(0).Rows(i).Item("CODE").ToString() = "COUNTRY" Then
                                tdInner.Text = "<b>" + dsLogicOR.Tables(0).Rows(i).Item("CATEGORY").ToString() + "</b><br/>(" + dsLogicOR.Tables(0).Rows(i).Item("DETAILS").ToString() + ")" + "<br/><b>State</b><br/>(" + Session("SearchState") + ")"
                        ...
                        Case 2
                            ...
                            If dsLogicOR.Tables(0).Rows(i).Item("CODE").ToString() = "COUNTRY" Then
                                If Session("SearchState") <> "All" Then
                                    dsCompany = objGetdata.GetSearchResults(Session("USERID").ToString(), "STATE")
                                Else
                                    dsCompany = objGetdata.GetSearchResults(Session("USERID").ToString(), "COUNTRY")
                                End If
                            Else
                                dsCompany = objGetdata.GetSearchResults(Session("USERID").ToString(), dsLogicOR.Tables(0).Rows(i).Item("CODE").ToString())
                            End If
                        ...
                        Case 3
                            If i = 0 Then
                                InnerTdSetting(tdInner, "", "Left")
                                dsCompany = New DataSet
                                dsCompany = objGetdata.GetLogicAndSearchResults(Session("USERID").ToString())
                                tbl = New Table()
                                If dsCompany.Tables(0).Rows.Count > 0 Then
                                    For k = 0 To dsCompany.Tables(0).Rows.Count - 1
                                        tr = New TableRow
