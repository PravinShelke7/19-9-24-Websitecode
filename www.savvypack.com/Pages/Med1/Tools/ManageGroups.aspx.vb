...
Partial Class Pages_MedEcon1_Tools_ManageGroups
    ...
    Protected Sub GetPCaseDetails()
        ...
        Try
            If Session("SavvyModType") <> "1" Then
                ds = objGetData.GetPCaseGrpDetailsBem(Session("Med1UserName").ToString(), Session("USERID").ToString(), txtKey.Text.Trim())
            Else
                If Session("Service") = "COMP" Then
                    ds = objGetData.GetCompCaseGrpDetails(Session("Med1UserName").ToString(), Session("USERID").ToString(), txtKey.Text.Trim(), Session("ServiceId"))
                Else
                    ds = objGetData.GetPCaseGrpDetails(Session("Med1UserName").ToString(), Session("USERID").ToString(), txtKey.Text.Trim())
                End If
    ...
    Protected Sub btnCreateGrp_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreateGrp.Click
        Try
            ...
            If Name.Length <> 0 Then
                ...
                Else
                    If Session("Service") = "COMP" Then
                        objUpdateData.AddCompGroupName(txtGroupDe1.Text, txtGroupDe2.Text, Session("UserId"), Session("ServiceId"))
                    Else
    ...
    Protected Sub btnGlobalManager_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnGlobalManager.Click
        Try
            Session("Med1GroupID") = Nothing
            'Response.Redirect("~/Pages/Econ1/Default.aspx")
            If Session("SavvyModType") <> "1" Then
                Response.Redirect("~/Pages/Med1/Default2.aspx")
            Else
