...
Partial Class Pages_MoldSustain1_Tools_ManageGroups
    ...
    Protected Sub GetPCaseDetails()
        ...
        Try
            ...
            If Session("SavvyModType") <> "1" Then
                ds = objGetData.GetPCaseGrpDetailsBem(Session("MoldS1UserName").ToString(), Session("USERID").ToString(), txtKey.Text.Trim())
            Else
                If Session("Service") = "COMPS1" Then
                    ds = objGetData.GetCompCaseGrpDetails(Session("MoldS1UserName").ToString(), Session("USERID").ToString(), txtKey.Text.Trim(), Session("MoldS1ServiceId").ToString())
                Else
                    ds = objGetData.GetPCaseGrpDetails(Session("USERID").ToString(), Session("USERID").ToString(), txtKey.Text.Trim())
                End If
    ...
    Protected Sub btnCreateGrp_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreateGrp.Click
        Try
            ...
            If Name.Length <> 0 Then
                ...
                Else
                    If Session("Service") = "COMPS1" Then
                        objUpdateData.AddCompGroupName(txtGroupDe1.Text, txtGroupDe2.Text, Session("UserId"), Session("MoldS1ServiceId"))
                    Else
    ...
    Protected Sub btnGlobalManager_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnGlobalManager.Click
        Try
            Session("MoldS1GroupID") = Nothing
            'Response.Redirect("~/Pages/Sustain1/Default.aspx")
            If Session("SavvyModType") <> "1" Then
                Response.Redirect("~/Pages/MoldSustain1/Default2.aspx")
            Else
