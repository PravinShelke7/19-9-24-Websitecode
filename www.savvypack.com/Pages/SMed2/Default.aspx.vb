...
Partial Class Pages_MedSustain2_Default
    Inherits System.Web.UI.Page
#Region "Browser Refresh Check"
    Dim objRefresh As zCon.Net.Refresh

    Protected Sub Page_PreInit(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreInit
        objRefresh = New zCon.Net.Refresh("_PAGES_MEDSUSTAIN2_DEFAULT")
    End Sub
...
#End Region
    ...
    Protected Sub SetToolButton()
        Try
            If Session("SMed2ServiceRole") <> "ReadWrite" Then
    ...
    Protected Sub SetSessions()
        ...
        Try
            ...
            Session("SMed2UserRole") = ds.Tables(0).Rows(0).Item("USERROLE").ToString()
            Session("SMed2ServiceRole") = ds.Tables(0).Rows(0).Item("SERVIECROLE").ToString()
            Session("SMed2UserName") = ds.Tables(0).Rows(0).Item("USERNAME").ToString()
            Session("SMed2ToolUserName") = ds.Tables(0).Rows(0).Item("TOOLUSERNAME").ToString()
            Session("SMed2MaxCaseCount") = ds.Tables(0).Rows(0).Item("MAXCASECOUNT").ToString()
            'Set for License Administrator
            Session("SMed2LicAdmin") = ds.Tables(0).Rows(0).Item("ISIADMINLICUSR").ToString()
            Session("SMed2LicenseId") = ds.Tables(0).Rows(0).Item("LICENSEID").ToString()
        Catch ex As Exception
    ...
    Protected Sub btnBCase_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBCase.Click
        Try
            Session("SMed2CaseId") = ddlBaseCases.SelectedValue.ToString()
    ...
    Protected Sub btnPCase_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPCase.Click
        Try
            Session("SMed2CaseId") = ddlPCase.SelectedValue.ToString()
            If Not objRefresh.IsRefresh Then
    ...
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            obj.CaseRename(txtCaseid.Value, txtCaseDe1.Text.Trim().ToString().Replace("'", "''"), txtCaseDe2.Text.Trim().ToString().Replace("'", "''"), txtCaseDe3.Text.Trim.ToString().Replace("'", "''"), "MedSustain2ConnectionString")
            GetPCaseDetails()
