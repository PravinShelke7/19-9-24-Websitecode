...
Partial Class Charts_Med2Charts_CSProftAndLoss
...
#End Region
    ...
    Protected Sub GetPerUnit(ByVal DsData As DataTable, ByVal DsPref As DataTable)
        Try
            ...
            pcScript &= "" + Graphtype + ".transposed(true)" + Graphtype + ".setCategories(Revenue;Variable Margin;Plant Margin)"
            For i = 0 To Cnt
                ...
                If IsDep = "N" Then
                    ...
                    Revenue = FormatNumber((DsData.Rows(i).Item("REVENUE") * DsPref.Rows(0).Item("curr")) * 100 / SalesVolumeUnit, 3)
                    VariableMargin = FormatNumber((DsData.Rows(i).Item("VM") * DsPref.Rows(0).Item("curr")) * 100 / SalesVolumeUnit, 3)
                    PlantMargin = FormatNumber((DsData.Rows(i).Item("PM") * DsPref.Rows(0).Item("curr")) * 100 / SalesVolumeUnit, 3)
                Else
                    Revenue = FormatNumber((DsData.Rows(i).Item("REVENUE") * DsPref.Rows(0).Item("curr")) * 100 / SalesVolumeUnit, 3)
                    VariableMargin = FormatNumber((DsData.Rows(i).Item("VM") * DsPref.Rows(0).Item("curr")) * 100 / SalesVolumeUnit, 3)
                    PlantMargin = FormatNumber((DsData.Rows(i).Item("PMDEP") * DsPref.Rows(0).Item("curr")) * 100 / SalesVolumeUnit, 3)

                End If


                pcScript &= "" + Graphtype + ".setSeries(" & DsData.Rows(i).Item("CASEDE") & ";" & Revenue & ";" & VariableMargin & ";" & PlantMargin & ")"
            Next
    ...
    Protected Sub GetPerWeight(ByVal DsData As DataTable, ByVal DsPref As DataTable)
        Try
            ...
            For i = 0 To Cnt
                ...
                If IsDep = "N" Then
                    Revenue = FormatNumber((DsData.Rows(i).Item("REVENUE") * DsPref.Rows(0).Item("curr")) / SalesVolumeLb, 3)
                    VariableMargin = FormatNumber((DsData.Rows(i).Item("VM") * DsPref.Rows(0).Item("curr")) / SalesVolumeLb, 3)
                    PlantMargin = FormatNumber((DsData.Rows(i).Item("PM") * DsPref.Rows(0).Item("curr")) / SalesVolumeLb, 3)
                Else
                    Revenue = FormatNumber((DsData.Rows(i).Item("REVENUE") * DsPref.Rows(0).Item("curr")) / SalesVolumeLb, 3)
                    VariableMargin = FormatNumber((DsData.Rows(i).Item("VM") * DsPref.Rows(0).Item("curr")) / SalesVolumeLb, 3)
                    PlantMargin = FormatNumber((DsData.Rows(i).Item("PMDEP") * DsPref.Rows(0).Item("curr")) / SalesVolumeLb, 3)
                End If
    ...
    Protected Sub GetTotal(ByVal DsData As DataTable, ByVal DsPref As DataTable)
        Try
            ...
            For i = 0 To Cnt
                ...
                If IsDep = "N" Then
                    Revenue = FormatNumber(DsData.Rows(i).Item("REVENUE") * DsPref.Rows(0).Item("curr"), 3)
                    VariableMargin = FormatNumber(DsData.Rows(i).Item("VM") * DsPref.Rows(0).Item("curr"), 3)
                    PlantMargin = FormatNumber(DsData.Rows(i).Item("PM") * DsPref.Rows(0).Item("curr"), 3)
                Else
                    Revenue = FormatNumber(DsData.Rows(i).Item("REVENUE") * DsPref.Rows(0).Item("curr"), 3)
                    VariableMargin = FormatNumber(DsData.Rows(i).Item("VM") * DsPref.Rows(0).Item("curr"), 3)
                    PlantMargin = FormatNumber(DsData.Rows(i).Item("PMDEP") * DsPref.Rows(0).Item("curr"), 3)
                End If
