...
Partial Class Charts_E4Charts_CPrftCost
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim objE4GetData As New E4GetData.Selectdata
        Dim i As Integer = 0
        Dim flag = False
        Dim flagVal = True
        ...
        Try
            ...
            Else
                ...
                Else
                    ...
                    dtCatSet = objE4GetData.GetCategorySetByID(ddlCategorySet.SelectedValue)
                    If dtCatSet.Rows.Count > 0 Then
                        dt = objE4GetData.GetCategoryBySet(ddlCategorySet.SelectedValue, "")
                        If dt.Rows.Count > 0 Then
                            For i = 0 To dt.Rows.Count - 1
                                ds = objE4GetData.GetCategoryItemBycategory(dt.Rows(i).Item("CATEGORYID").ToString())
        ...
        Try
            If PrftCost = "PRFT" Then
                If ChartType = "RBC" Then
                    Page.Title = "E4-Combine Profit And Loss Bar Chart"
                    rwType.Visible = True
                Else
                    Page.Title = "E4-Combine Profit And Loss  Stack Chart"
                    rwType.Visible = False
                End If
            Else
                If ChartType = "RBC" Then
                    Page.Title = "E4-Combine Cost Bar Chart"
                    rwType.Visible = True
                Else
                    Page.Title = "E4-Combine Cost Stack Chart"
                    rwType.Visible = False
                End If
            End If
        Catch ex As Exception
            ...
            ElseIf ddlChartType.SelectedValue = "SBAR" Then
                ...
                If (ddlCategorySet.SelectedValue = "0" Or ddlCategorySet.SelectedValue = "") Then
                    ...
                    For i = 0 To Cnt
                        ...
                        If SalesVolumeUnit <> 0 Then
                            matCost = FormatNumber(DsData.Rows(i).Item("VMATERIAL") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)
                            labCost = FormatNumber(DsData.Rows(i).Item("VLABOR") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)
                            ergyCost = FormatNumber(DsData.Rows(i).Item("VENERGY") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)
                            DPCost = FormatNumber(DsData.Rows(i).Item("VPACK") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)
                            ShipCost = FormatNumber(DsData.Rows(i).Item("VSHIP") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)

                            If PrftCost = "PRFT" And IsDep = "N" Then
                                fixedCost = FormatNumber(DsData.Rows(i).Item("FIXEDCOST") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)
                                plantM = FormatNumber(DsData.Rows(i).Item("PM") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)
                            Else
                                fixedCost = FormatNumber(DsData.Rows(i).Item("FIXEDCOSTDEP") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)
                                plantM = FormatNumber(DsData.Rows(i).Item("PMDEP") * DsPref.Rows(0).Item("curr") * 100 / SalesVolumeUnit, 4)
                            End If
                        End If
    ...
    Protected Sub ddlPName_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPName.SelectedIndexChanged
        Try
            Bindddl()
        Catch ex As Exception
            ErrorLable.Text = "Error:Selected Index Changed:" + ex.Message.ToString() + ""
        End Try
    End Sub
    ...
    Function GetStackbarChartDetails(ByVal DsData As DataTable, ByVal DsPref As DataTable, ByVal Contype As String) As String
        Dim ObjGetE4data As New E4GetData.Selectdata
        ...
        Try
            ...
            dsCategories = ObjGetE4data.GetCategoryBySet(ddlCategorySet.SelectedValue, "")



            For k = 0 To Cnt

                If DsData.Rows(k).Item("FINVOLMUNITS") > 0 Then
                    SalesVolumeUnit = DsData.Rows(k).Item("FINVOLMUNITS")
                End If

                If CDbl(DsData.Rows(k).Item("PVOLUSE")) = 0.0 Then
                    SalesVolumeLb = DsData.Rows(k).Item("VOLUME") * DsPref.Rows(0).Item("CONVWT")
                Else
                    SalesVolumeLb = DsData.Rows(k).Item("SVOLUME") * DsPref.Rows(0).Item("CONVWT")
                End If

                pcsetSeries = pcsetSeries + "" + Graphtype + ".setSeries(" & DsData.Rows(k).Item("CASEID") & ";"
                For i = 0 To dsCategories.Rows.Count - 1
                    dsitems = ObjGetE4data.GetCategoryItemBycategory(dsCategories.Rows(i).Item("Categoryid").ToString())
