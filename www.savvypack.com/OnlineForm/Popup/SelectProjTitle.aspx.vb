...
Partial Class OnlineForm_Popup_SelectProjTitle
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            hidPrjDes.Value = Request.QueryString("Des1").ToString()
            If Not IsPostBack Then
                'Started Activity Log Changes
                Try
                    objUpIns.InsertLog1(Session("UserId").ToString(), "SelectProjTitle.aspx", "Opened Select Project Title Popup to select Project for sending message", "", Session("SPROJLogInCount").ToString())
        ...
        End Try
    End Sub
    Protected Sub getProjDetails(ByVal Title As String)
        Dim ds As New DataSet
        Dim objGetData As New Selectdata
        Try
            ds = objGetData.GetProjTitle(Session("UserId"), Title.ToString())
            Session("ds") = ds
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        Try
            getProjDetails(txtUser.Text.Trim.ToString().Replace("'", "''"))
            'Started Activity Log Changes
            Try
                objUpIns.InsertLog1(Session("UserId").ToString(), "SelectProjTitle.aspx", "Clicked on Search Button, Searched Text:" + txtUser.Text.ToString().Replace("'", "''"), "", Session("SPROJLogInCount").ToString())
