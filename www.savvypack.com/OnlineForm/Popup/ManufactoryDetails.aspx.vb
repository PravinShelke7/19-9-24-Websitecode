...
Partial Class Pages_PopUp_ManufactoryDetails
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                If Request.QueryString("Type") = "N" Then
                    ...
                    Try
                        objUpIns.InsertLog1(Session("UserId").ToString(), "ManufactoryDetails.aspx", "Opened Manufactory Details Popup for Project:" + Session("ProjectId") + "", Session("ProjectId"), Session("SPROJLogInCount").ToString())
                ...
                Else
                    ...
                    'Started Activity Log Changes
                    Try
                        objUpIns.InsertLog1(Session("UserId").ToString(), "ManufactoryDetails.aspx", "Opened Edit Manufactory Details Popup for Project:" + Session("ProjectId") + " and Manufactory:" + txtName.Text + "", Session("ProjectId"), Session("SPROJLogInCount").ToString())
    ...
    Protected Sub PageDetails()
        ...
        Try
            If Request.QueryString("Type") = "E" Then
                ds = objGetData.GetManufactoryDetails(Request.QueryString("ManufactoryId").ToString(), Request.QueryString("ProjectId").ToString())
            ...
            tblManuf.Controls.Add(trHeader)
                ...
                For j = 1 To 9
                    ...
                    Select Case j
                        ...
                        Case 2
                            ...
                            txt.Width = 100
                            txt.MaxLength = 100
                            txt.ToolTip = "Eg: Blow molding, Slittting, Injection molding, Coating, etc"
        ...
        Try
            ...
            If Request.QueryString("Type") = "N" Then
                If txtName.Text <> "" Then
                    ds = objGetData.GetModelValueDetails(Session("ProjectId"), "13", txtName.Text.Replace("'", "''"))
                    If ds.Tables(0).Rows.Count = 0 Then
                        objUpIns.AddManufactoryDetails(Session("ProjectId"), txtName.Text.Replace("'", "''"), txtOSize.Text.Replace("'", "''"), txtRSize.Text.Replace("'", "''"), PP, Mac, Rate, TPut, DTime, Waste, CC, Info)
                        seq = objUpIns.InsertProjCategoryDetails(Session("ProjectId"), "13", txtName.Text.Replace("'", "''"))

                        'Started Activity Log Changes
                        Try
                            objUpIns.InsertLog1(Session("UserId").ToString(), "ManufactoryDetails.aspx", "Added New Manufactory Details for Project:" + Session("ProjectId") + "", Session("ProjectId"), Session("SPROJLogInCount").ToString())
                            ...
                            For i = 1 To ht.Count
                                For Each item As Object In ht
                                    ...
                                    If str(1) = i Then
                                        objUpIns.EditInsertLog(Session("UserId").ToString(), "11", str(0), seq.ToString(), item.Value.ToString().Replace("'", "''"), Session("ProjectId"), Session("SPROJLogInCount").ToString())
                                        Exit For
                                    End If
                                Next
                            Next
                    ...
                    Else
                        PageDetails()
                        Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinRename", "alert('Manufactory Name " + txtName.Text + " already exist.');", True)
                    End If
                Else
                    PageDetails()
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinRename", "alert('Manuctory Details not Added Successfully.Please Enter Manufactory Name.');", True)
                End If

            Else
                If txtName.Text <> "" Then
                    ds = objGetData.GetExistManufactoryDetails(txtName.Text.Replace("'", "''"), Request.QueryString("ProjectId").ToString(), Request.QueryString("ManufactoryId").ToString())
                    If ds.Tables(0).Rows.Count = 0 Then
                        objUpIns.UpdateManufactoryDetails(Request.QueryString("ManufactoryId").ToString(), Request.QueryString("ProjectId").ToString(), txtName.Text.Replace("'", "''"), txtOSize.Text.Replace("'", "''"), txtRSize.Text.Replace("'", "''"), PP, Mac, Rate, TPut, DTime, Waste, CC, Info)
                        objUpIns.UpdateProjCategoryDetails(Session("ProjectId"), "13", txtName.Text.Replace("'", "''"), Request.QueryString("ProjCatId").ToString())

                        dsData = objGetData.GetExistingModelDetails(Session("ProjectId"))

                        If dsData.Tables(0).Rows.Count > 0 Then
                            For i = 0 To dsData.Tables(0).Rows.Count - 1
                                If dsData.Tables(0).Rows(i).Item("MANUFACTORY").ToString() = hidName.Value Then
                                    objUpIns.EditModelDetail(dsData.Tables(0).Rows(i).Item("MODELID").ToString(), Session("ProjectId"), txtName.Text.Replace("'", "''"), "MANUF")
                                End If
                            Next
                        End If
                        ...
                        'Started Activity Log Changes
                        Try
                            objUpIns.InsertLog1(Session("UserId").ToString(), "ManufactoryDetails.aspx", "Edited Manufactory Details for Project:" + Session("ProjectId") + " and Manufactory:" + txtName.Text.Replace("'", "''") + "", Session("ProjectId"), Session("SPROJLogInCount").ToString())
                            Dim ht As Hashtable = New Hashtable()
                            If Session("ht") IsNot Nothing Then
                                ht = DirectCast(Session("ht"), Hashtable)
                            End If
                            ...
                            For i = 1 To ht.Count
                                For Each item As Object In ht
                                    str = item.Key.ToString().Split("-")
                                    If str(1) = i Then
                                        objUpIns.EditInsertLog(Session("UserId").ToString(), "11", str(0), Request.QueryString("ManufactoryId").ToString(), item.Value.ToString().Replace("'", "''"), Session("ProjectId"), Session("SPROJLogInCount").ToString())
                                        Exit For
                                    End If
                                Next
                            Next
                    ...
                    Else
                        PageDetails()
                        Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinRename", "alert('Manufactory name Already Exist.');", True)
                    End If

                Else
                    PageDetails()
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinRename", "alert('Manufactory Details not Added Successfully.Please Enter Manufactory Name.');", True)
                End If
            End If
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        Try
            objUpIns.DeleteManufactoryDetails(Request.QueryString("ManufactoryId").ToString, Request.QueryString("ProjectId").ToString, hidName.Value.Replace("'", "''"))
            objUpIns.DeleteProjCategoryDetails(Session("ProjectId"), "13", txtName.Text.Replace("'", "''"), Request.QueryString("ProjCatId").ToString)
            ds = objGetData.GetExistingModelDetails(Session("ProjectId"))
            dv = ds.Tables(0).DefaultView()
            dv.RowFilter = "MANUFACTORY='" + hidName.Value.Replace("'", "''") + "'"

            dt = dv.ToTable()
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    objUpIns.DeleteModelByType(Request.QueryString("ProjectId").ToString(), dt.Rows(i).Item("MODELID").ToString(), "MANUF", hidName.Value.Replace("'", "''"))
                Next
            End If
            'Started Activity Log Changes
            Try
                objUpIns.InsertLog1(Session("UserId").ToString(), "ManufactoryDetails.aspx", "Deleted Manufactory Details for Project:" + Session("ProjectId") + "", Session("ProjectId"), Session("SPROJLogInCount").ToString())

            Catch ex As Exception

            End Try
            'Ended Activity Log Changes
            Page.ClientScript.RegisterStartupScript(Me.GetType(), "New_WinRename", "alert('Manufactory deleted Successfully.');", True)
            Page.ClientScript.RegisterStartupScript(Me.GetType(), "ClosePage", "ClosePage('E')", True)
        Catch ex As Exception
