...
function checkNumericForPallet() {
    var txtarray = document.getElementsByTagName("input");
    var flag;
    var anum = /(^\d+$)|(^\d+\.\d+$)/
    for (var i = 0; i < txtarray.length; i++) {
...
function CheckForFixedCost(MasterId, Txt,Pref) {
    //alert(Flag);
    var IsNumflag;
    var isCostFlag;
    //Checking Preferred Value
    var anum = /(^\d+$)|(^\d+\.\d+$)/
            ...
            for (var i = 1; i <= 30; i++)
             {
                 var TxtId = MasterId + "_" + Txt + i;
                 var PrefId = MasterId + "_" + Pref + i;
