...
function CheckForMaterialPage(MasterId, Mat,Weight,Dep, DepName,PComp) 
{
    var IsNumflag;   
    var IsDep;
    var IsWeight;
    ...
    if (IsNumflag) {
        ...
        for (var i = 1; i <= 10; i++) {
            ...
            var WeightVal = document.getElementById(WeightId).value;
             var PackComp= MasterId + "_" + PComp + i;           
            var PackCompVal= document.getElementById(PackComp).checked;
            if (MatVal > 0) 
            {
                IsWeight=true;
                if(PackCompVal==false)
                {
                   IsPackComp=true;
                }
            } 
    ...
    if (IsNumflag && IsDep) 
    {
        ...
        {
           alert("Update did not complete, please uncheck at least one Package Component.");
          return false;
        }
    }
    else 
...
function CheckForSEquipmentPage(MasterId, Equip, CstType, CstTypeName,EqNum) {
    ...
    function CheckForFixedCost(MasterId, Txt) {
        //alert(Flag);
        var IsNumflag;
        var IsDep;
        var isCostFlag;
